...
help:
	...
	@echo ""
	@echo "  make architecture-audit-full - Run complete master enterprise architecture audit"
	@echo "  make install     - Install dependencies"
	@echo "  make clean       - Clean build artifacts"
	@echo ""

.PHONY: architecture-audit-full architecture-independent-builds architecture-scanner-tests architecture-secret-scan architecture-accessibility architecture-dead-code production-item-sync-verification

architecture-audit-full:
	./scripts/python.sh scripts/architecture/run_full_architecture_audit.py

production-item-sync-verification:
	./scripts/python.sh scripts/architecture/verify_item_sql_sync.py

architecture-independent-builds:
	./scripts/python.sh scripts/architecture/independent_builds.py

architecture-scanner-tests:
	./scripts/python.sh scripts/architecture/scanner_priority_test.py

architecture-secret-scan:
	./scripts/python.sh scripts/architecture/secret_scan.py

architecture-accessibility:
	./scripts/python.sh scripts/architecture/accessibility_audit.py

architecture-dead-code:
	./scripts/python.sh scripts/architecture/dead_code_scan.py
...
node-test:
	...
	cd frontend && node ./scripts/run-jest.cjs
...
node-e2e-recount-smoke:
	...
	cd frontend && E2E_BACKEND_URL=http://127.0.0.1:8001 pnpm run e2e:recount-smoke
