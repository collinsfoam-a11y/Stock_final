...
## 2024-08-07 - Add decorative props to redundant icon elements
**Learning:** In composite interactive components (like Settings Action Rows or Text Input Rows), when the parent wrapper or semantic label already describes the state to screen readers, child visual icons must have `getDecorativeIconProps()` applied to prevent redundant or confusing audio clutter.
**Action:** Always apply `{...getDecorativeIconProps()}` to `Ionicons` or other structural/visual icons that do not provide additional semantic value beyond what is already described by text or parent accessibility props.
