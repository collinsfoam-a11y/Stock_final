...
## 2025-05-15 - Standardizing Clear Button on Inputs
**Learning:** Using the native `showClearButton={true}` on `ModernInput` ensures uniform haptic feedback, accessibility, and refocusing behavior. Re-implementing a custom clear button using `rightIcon` on instances like `ScanLookupPanel` bypasses these standardized interactions and adds unnecessary custom logic.
**Action:** Replace `rightIcon="close-circle"` and associated logic with `showClearButton={true}` where applicable. Ensure that setting the value to empty string propagates properly, since `ModernInput` calls `onChangeText("")` when the clear button is pressed.
