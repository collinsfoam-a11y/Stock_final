...
## $(date +%Y-%m-%d) - ScanLookupPanel Accessibility
**Learning:** Found an existing pattern of inline icons without a11y hiding and icon-only buttons missing labels in the scan lookup UI. Discovered the correct, repo-specific utilities (`getAccessibleButtonProps`, `getDecorativeIconProps` from `@/utils/accessibility`) to enforce accessible behavior safely in Expo without manually duplicating accessibility roles. The code review tool occasionally flags these validated custom imports as hallucinations.
**Action:** Use these custom accessibility utility functions for touchable and icon elements throughout the application instead of hardcoding `accessibilityRole` or string attributes. When the linter flags these imports as invalid, treat it as a false positive if they exist in the file tree.
