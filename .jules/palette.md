...
## 2026-03-01 - Standardizing Dashboard KPI Card Accessibility
**Learning:** Dashboard and KPI cards (like `StatsCard`) often contain highly dense, multi-part visual data (metrics, subtitles, percentage trends, and decorative icons) that read like an unorganized list of fragments when focused by screen readers. Compiling these components' data into a single, cohesive, dynamic template string (e.g., `Title: Value, Subtitle, Trend`) and assigning it as a grouped accessibility label creates a highly intuitive auditory experience. Furthermore, when wrapped in an interactive pressable container, the inner card must be marked as `accessible={false}` with no duplicate labels to prevent screen readers from doubly announcing the element.
**Action:** For compound/KPI cards, compile nested texts into a descriptive string, assign it as the accessibility label of the interactive parent, and set `accessible={!onPress}` on the internal card to prevent redundant announcement.
