...
## 2024-03-24 - EmptyState Accessibility Label Interpolation
**Learning:** In the `EmptyState` component, string interpolation using `logical OR` (`||`) for `accessibilityLabel` fallbacks caused screen readers to announce a trailing period (e.g., `"Title. "`) when the optional `message` prop was omitted. This occurs because `"Title. "` is considered truthy.
**Action:** When constructing fallback string interpolations that combine optional fields with punctuation, always use defensive ternary checks (e.g., `message ? \`${title}. ${message}\` : title`) combined with the nullish coalescing operator (`??`) to ensure punctuation is only added when the optional text is present, and explicit empty strings are respected.
