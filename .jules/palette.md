...
## 2026-05-25 - Standardizing Haptics and Accessibility on Search Results
**Learning:** Search result items and their paginated "Load More" controls are primary interaction targets in operational lists. By default, they lack both tactile confirmation on press and explicit, descriptive screen reader labels (often falling back to generic "Item" or just reading the raw child text). Hard-coded numeric layout values (e.g., `padding: 12`) in these lists also trigger UI governance linter failures. Additionally, when refactoring components to use unified theme design tokens, missing mock definitions in associated test files (`__tests__`) can crash the test suite.
**Action:** Always apply `void haptics.light()` to primary list item and pagination interactions. Use `getAccessibleButtonProps` to construct highly descriptive labels (e.g., `Select item: ${itemName}`). Replace hard-coded layout numbers with centralized theme tokens (e.g., `spacing.md`), and ensure any new tokens introduced are mocked in the corresponding test files to maintain CI stability.
