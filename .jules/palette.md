...
## 2026-05-23 - Satisfying UI Governance Linter for Accessible Touchables
**Learning:** The custom frontend UI governance linter (`check-ui-governance.cjs`) is strict and will flag direct touchable components (like `TouchableOpacity`) for lacking explicit accessibility props even if `accessibilityRole` and `accessibilityLabel` are manually provided. It specifically looks for the `getAccessibleButtonProps()` utility to ensure complete accessibility compliance (including `hitSlop` and dynamic `accessibilityState`).
**Action:** Always spread `getAccessibleButtonProps()` on custom touchable wrappers (like `TouchableOpacity`) instead of manually applying individual accessibility props, to satisfy both screen readers and internal governance tools.
