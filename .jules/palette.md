...
## 2026-08-05 - Search Autocomplete Haptics and Accessibility
**Learning:** Dropdown autocomplete components often have rich interactivity (selecting items, clearing text, loading more) but lack tactile feedback and screen reader context. Using `getAccessibleButtonProps` on list items allows propagating dynamic labels and the `selected` state to screen readers. Adding haptics (`haptics.selection()` for item choice, `haptics.light()` for clearing or loading) drastically improves the perceived responsiveness of search interactions. Applying `getDecorativeIconProps` to clear icons ensures screen readers don't read redundant graphical elements.
**Action:** When implementing or refactoring custom autocomplete or dropdown components, ensure every interactive element uses the centralized `haptics` service and applies full accessibility attributes (`getAccessibleButtonProps`/`getDecorativeIconProps`) to define state (`selected`, `busy`, `disabled`) and intent clearly.
