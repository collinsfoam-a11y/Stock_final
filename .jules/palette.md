...
## 2026-05-15 - [Clear Button for Form Inputs] (Updated)
**Learning:** Adding a clear button to form inputs is a high-impact micro-UX improvement that reduces friction during data correction. When replacing manual `rightIcon` clear implementations with `ModernInput`'s built-in `showClearButton={true}`, the component's internal `handleClear` automatically calls `onChangeText("")`. Therefore, parent components must handle the empty string case in `onChangeText` to perform any necessary side effects (like dismissing error notices) that were previously handled by dedicated `onClear` callbacks.
**Action:** Always prefer `showClearButton={true}` over manual `rightIcon` implementations for clear actions in `ModernInput`. Ensure the parent's `onChangeText` handler checks for empty strings to perform necessary cleanup logic.
