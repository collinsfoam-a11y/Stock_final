...
## 2026-08-19 - Standardizing Clear Button Interaction in Autocomplete Inputs
**Learning:** Clear buttons inside complex input components (like SearchAutocomplete) require the same level of accessibility and tactile attention as standalone inputs. It is crucial to spread `getAccessibleButtonProps` on the touch target wrapper and `getDecorativeIconProps` on the icon itself to satisfy UI governance checks and improve screen reader clarity, along with applying `haptics.light()` for tactile confirmation when the input is cleared.
**Action:** Always apply the standard micro-UX clear button pattern (`getAccessibleButtonProps`, `getDecorativeIconProps`, `haptics.light()`) when refactoring or enhancing custom autocomplete/search input components.
