...
## 2026-05-22 - Standardizing Tactile Feedback and Accessibility on Premium Headers
**Learning:** Icon-only buttons within `PremiumHeader` (such as actions, logout, or menus) were lacking appropriate screen reader labels and tactile confirmation, diminishing the premium feel and discoverability.
**Action:** When adding or modifying actions in `PremiumHeader.tsx` or similar app headers, always apply `getAccessibleButtonProps({ label: '...' })` to the `TouchableOpacity`, spread `getDecorativeIconProps()` on the enclosed `Ionicons`, and wrap the action inside the `onPress` callback with `haptics.light()` to ensure a consistent, accessible, and responsive user experience.
