...
## 2025-02-28 - Badge accessibilityLabel edge case handling
**Learning:** In the `Badge` component, using a simple truthy check `label ?` to determine if a label should be appended to the "Badge: " string causes issues when `label` is the numeric value `0`. Since `0` is falsy, the fallback resolves to just "Badge" instead of "Badge: 0".
**Action:** Use strict undefined and empty string checks (e.g., `label !== undefined && label !== ""`) when computing accessibility label fallbacks to ensure valid falsy values like `0` are correctly processed and announced by screen readers.
