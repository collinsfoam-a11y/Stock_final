...
## 2024-08-15 - Micro-UX on Form Input Clear Buttons
**Learning:** Purely decorative icons in interactable elements like clear buttons need their own explicitly mapped properties (`getDecorativeIconProps()`) on top of the button's standard accessibility labels (`getAccessibleButtonProps()`) to avoid screen reader clutter. Calling an asynchronous method like `haptics.light()` on button presses requires explicitly prepending it with `void` to gracefully swallow the unhandled promise lint warnings in the React Native environment.
**Action:** Always check the clear button components in our system. Implement `void haptics.light()`, add `getAccessibleButtonProps()`, and ensure inner purely visual icons have `getDecorativeIconProps()` spread onto them to guarantee complete and pleasing accessibility support.
