...
## 2026-05-25 - Enhancing DataTable Accessibility and Interactivity
**Learning:** Data tables often contain hidden interactive elements (like sortable column headers, clickable rows, and pagination controls) that are critical for navigation and operation but often lack appropriate screen reader context or tactile feedback. Ensuring these elements use `getAccessibleButtonProps` to announce their purpose (e.g., 'Sort by Column Name', 'Row 1', 'Next page') and `getDecorativeIconProps` to silence redundant icons significantly improves screen reader usability. Additionally, tying these interactions to `haptics.light()` provides immediate tactile confirmation of the action.
**Action:** When implementing or refactoring complex interactive components like tables, lists, or grids, systematically identify every touchable element and explicitly define its accessible label, decorative children, and haptic response using the project's centralized utilities.
