...
## 2024-08-19 - Push filtering to DB for counts
**Learning:** System reports previously pulled full documents using `find().to_list()` and counted them in Python memory, which is highly inefficient for large datasets.
**Action:** When determining lengths or verifying row counts matching specific conditions (like timestamps), use the native MongoDB query `count_documents(query)` instead of fetching full documents to improve speed and memory utilization.
