...
## 2024-06-23 - [Parallelize Independent DB Queries]
**Learning:** Sequential `await` statements for independent database queries create an N+1 performance bottleneck, causing the total response time to be the sum of all individual query latencies.
**Action:** Always use `asyncio.gather()` in Python (or `Promise.all()` in JS) to execute independent database fetches or external API calls concurrently, reducing wait time to the duration of the slowest single query.
