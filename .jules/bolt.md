...
## 2024-07-19 - Remove virtualization overhead from non-scrollable lists
**Learning:** Virtualized lists (`FlatList` or `FlashList`) with `scrollEnabled={false}` introduce unnecessary virtualization overhead without the benefits. It doesn't actually virtualize because it can't scroll, but the components still run the virtualization logic.
**Action:** When `scrollEnabled={false}` is needed, map over the data directly to render items within a standard `View` instead.
