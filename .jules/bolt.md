...
## 2026-06-10 - MongoDB Query Optimization with $facet
**Learning:** Combining multiple sequential `count_documents` and `aggregate` queries into a single aggregation pipeline using the `$facet` stage significantly reduces database round-trips.
**Action:** Consistently look for multiple queries that fetch data related to the same collection (like counts and grouped stats) and consolidate them into a single `$facet` pipeline.
