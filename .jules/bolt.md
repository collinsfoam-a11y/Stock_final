...
## 2026-09-08 - Pydantic Model Instantiation Awaits
**Learning:** In Python, when passing multiple `await` expressions as keyword arguments to instantiate a Pydantic model (e.g., `KPIResponse(a=await func1(), b=await func2())`), the coroutines are evaluated sequentially. This creates an unexpected I/O bottleneck when the functions are independent database queries.
**Action:** To evaluate them concurrently, use `asyncio.gather()` to fetch the results first, then pass the destructured results to the model instantiation.
