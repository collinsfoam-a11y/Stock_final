...
export const localDb = {
  ...
  async getSessionStats(sessionId: string): Promise<{
    ...
    const { getProjectedSessionStatsBySessionIds } =
      await import("@/data/repositories/inventoryControlPlaneRepository");
    const projectedStats = (await getProjectedSessionStatsBySessionIds([sessionId])) as any;
    const db = await getDb();
