...
const cacheDetectedPort = (port: number): number => {
  cachedPort = port;
  return port;
};
...
export const detectFrontendPort = async (): Promise<number | null> => {
  ...
  if (fromWindow) return cacheDetectedPort(fromWindow);

  const fromExpo = detectPortFromExpoHostUri();
  if (fromExpo) return cacheDetectedPort(fromExpo);

  const probedPort = await probeFrontendPort();
  if (probedPort) return cacheDetectedPort(probedPort);

  const fallbackPort = getPlatformDefaultPort();
  if (Platform.OS === "web") {
    return cacheDetectedPort(fallbackPort);
  }
  return cacheDetectedPort(fallbackPort);
};
