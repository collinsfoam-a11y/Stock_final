...
describe("httpClient error notification helpers", () => {
  ...
  describe("getErrorMessageForStatus", () => {
    ...
    it("returns 400 default message", () => {
      expect(getErrorMessageForStatus(400, {})).toBe("Invalid request. Please check your input.");
    });

    it("returns 403 default message", () => {
      expect(getErrorMessageForStatus(403, {})).toBe("You don't have permission for this action.");
    });

    it("returns 500 default message", () => {
      expect(getErrorMessageForStatus(500, {})).toBe("Server error. Please try again later.");
    });
