...
const log = createLogger("backendUrl");
...
export const resolveBackendUrl = async (): Promise<string> => {
  ...
  if (currentOrigin) {
    ...
    if (isCurrentOriginHealthy) {
      log.info("Reusing healthy same-origin backend", { origin: currentOrigin });
  ...
  log.info("Probing backend candidates", { candidateCount: candidates.length });

  for (const url of candidates) {
    const isHealthy = await timeoutFetch(`${url}${HEALTH_PATH}`, 3000);
    if (!isHealthy) continue;

    log.info("Resolved healthy backend", { url });
    resolvedBackendUrl = url;
    return url;
  }

  // Fallback if nothing responds
  log.warn("No healthy backend found; using fallback", { fallback: BACKEND_URL });
...
export const initializeBackendURL = async (): Promise<string> => {
  ...
  log.info("Backend URL initialized", { url });
  return url;
};
