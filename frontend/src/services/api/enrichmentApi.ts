...
const pickEnrichmentFields = (
  data: EnrichmentData | Record<string, unknown>
): Partial<EnrichmentData> => {
  const result: Record<string, unknown> = {};
  for (const key of ENRICHMENT_WHITELIST) {
...
export const enrichItem = async (
  ...
  try {
    const response = await apiClient.post(`${ENRICHMENT_BASE}/record`, {
      item_code: itemCode,
      ...pickEnrichmentFields(enrichmentData),
    });

    const payload = response.data;
    const fieldsUpdated = Array.isArray(payload?.fields_updated) ? payload.fields_updated : [];
  ...
  try {
    const payload = {
      enrichments: enrichments.map((entry) => ({
        ...
        ...pickEnrichmentFields(entry.enrichment || {}),
      })),
    };
