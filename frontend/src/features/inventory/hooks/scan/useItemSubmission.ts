...
export const useItemSubmission = ({
  ...
  const returnToScan = () =>
    safeBackNavigation(router, {
      sessionFallbackHref: sessionId
        ? `/staff/scan?sessionId=${encodeURIComponent(sessionId)}`
        : undefined,
      userRole: "staff",
    });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
      Alert.alert("Success", "Item counted successfully", [{ text: "OK", onPress: returnToScan }]);
    } catch {
