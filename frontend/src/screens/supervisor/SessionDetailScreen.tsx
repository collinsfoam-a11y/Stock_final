...
import {
  finalizeSession,
  getFinalizationAssessment,
} from "@/services/api/api";
import type { FinalizationAssessmentDTO } from "@/services/api/inventoryWorkflowApi";
import { FinalizationGateChecklist } from "@/components/operational/FinalizationGateChecklist";
import { toFinalizationGateViewModel } from "@/viewModels/finalizationAdapter";
import { toExceptionViewModel } from "@/viewModels/exceptionAdapter";
import type { ExceptionViewModel } from "@/viewModels/types";
import { ExceptionRouter } from "@/components/operational/ExceptionRouter";
...
export default function SessionDetail() {
  ...
  const [finalizationAssessment, setFinalizationAssessment] = React.useState<FinalizationAssessmentDTO | null>(null);
  const [finalizationException, setFinalizationException] = React.useState<ExceptionViewModel[]>([]);
  ...
  const loadData = React.useCallback(async () => {
    ...
    try {
      ...
      const canonicalStatus = sessionStatus === "RECONCILE" ? "REVIEW" : sessionStatus;
      if (!offlineMode && canonicalStatus === "REVIEW") {
        try {
          const assessment = await getFinalizationAssessment(targetSessionId);
          setFinalizationAssessment(assessment);
          setFinalizationException([]);
        } catch (err: any) {
          const vm = toExceptionViewModel({
            code: err?.response?.data?.error_code || err?.code || "FINALIZATION_ASSESSMENT_FAILED",
            errorCode: err?.response?.data?.error_code,
            message: err?.response?.data?.detail || err.message || "Failed to load finalization assessment",
          });
          setFinalizationException([vm]);
          setFinalizationAssessment(null);
        }
      }
    } catch {
  ...
  }, [show, targetSessionId, offlineMode]);
  ...
  const handleUpdateStatus = async (newStatus: string) => {
   ...
   const handleFinalizeSession = async (assessmentToken?: string) => {
    if (offlineMode) {
    ...
    try {
      ...
      await finalizeSession(targetSessionId, { assessment_token: assessmentToken });
      await loadData();
      show("Session finalized", "success");
      setFinalizationException([]);
    } catch (error: any) {
      const vm = toExceptionViewModel({
        code: error?.response?.data?.error_code || "FINALIZE_REJECTED",
  ...
  const ListHeader = () => (
    <View>
      ...
      {session.status === "RECONCILE" && !offlineMode ? (
        <Animated.View entering={getFadeInDown(200)} style={styles.actionButtons}>
          {finalizationException.length > 0 ? (
            <ExceptionRouter exceptions={finalizationException} />
          ) : finalizationAssessment ? (
            <FinalizationGateChecklist
              vm={toFinalizationGateViewModel(finalizationAssessment)}
              onFinalize={(token?: string) => void handleFinalizeSession(token)}
            />
          ) : null}
        </Animated.View>
      ) : null}
