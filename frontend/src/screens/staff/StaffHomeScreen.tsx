...
const StaffHome = React.memo(function StaffHome() {
  ...
  const handleStartSession = async () => {
    ...
    try {
      ...
      toastService.showError(err instanceof Error ? err.message : "Failed to create session");
    } finally {
      setIsCreating(false);
    }
  };
  ...
  const handleLogout = () => {
    if (Platform.OS === "web" && typeof window !== "undefined") {
      if (window.confirm("Sign out of Stock Verify?")) {
        logout().finally(() => router.replace("/welcome" as any));
      }
      return;
    }
