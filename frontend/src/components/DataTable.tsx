      ...
      <View style={styles.tableWrapper}>
        ...
        >
          <View style={styles.tableContent}>
            {/* ⚡ Bolt Optimization: Replaced FlashList with simple array map.
                FlashList with scrollEnabled={false} adds virtualization overhead without the benefits. */}
            {paginatedData.map((item, index) => {
              const keyParts = columns.map((col) => String(item[col.key] || "")).join("-");
              const itemKey = `row-${index}-${keyParts.substring(0, 30)}`;
              return <React.Fragment key={itemKey}>{renderRow(item, index)}</React.Fragment>;
            })}
          </View>
        </ScrollView>
      </View>
