...
const ItemVerification: React.FC<ItemVerificationProps> = ({
  ...
  const getVarianceColor = (variance?: number) => {
    if (!variance) return semanticColors.text.secondary;
    if (variance > 0) return colors.success[500];
    ...
    <View style={styles.container}>
          ...
          <View style={styles.labelContainer}>
            <View
              style={[
                ...
                {
                  backgroundColor:
                    sqlVerifiedQty !== undefined ? colors.success[500] : colors.error[500],
                },
              ]}
            />
      ...
      {mismatchFlag && (
        <View style={styles.warningContainer}>
          ...
          <Text style={styles.warningText}>Variance detected against cached data.</Text>
        </View>
      )}

      <View style={styles.footer}>
        <Text style={styles.lastVerified}>Last verified: {formatDateTime(lastVerifiedAt)}</Text>
        {sqlVerifiedQty === undefined && (
          <View style={{ flexDirection: "row", alignItems: "center" }}>
            <Ionicons
              ...
              style={{ marginRight: 4 }}
            />
            <Text style={[styles.notVerifiedText, { color: colors.error[500] }]}>
              Source Disconnected
            </Text>
          </View>
        )}
