...
jest.mock("../../../store/authStore", () => ({
  useAuthStore: () => ({
    user: { full_name: "Jane Admin", role: "admin" },
    logout: mockLogout,
  }),
}));
...
describe("AdminSidebar", () => {
  ...
  it("renders brand section, user profile, nav items, and logout button", () => {
    const { getByText, getByLabelText } = render(<AdminSidebar />);

    expect(getByText("Admin Control")).toBeTruthy();
    expect(getByText("Jane Admin")).toBeTruthy();
    expect(getByText("Administrator")).toBeTruthy();
    expect(getByLabelText("Logout")).toBeTruthy();
  });

  it("toggles group expansion on section header press", () => {
    const { getByLabelText } = render(<AdminSidebar />);

    const headerButton = getByLabelText("Access Control section");
  ...
  it("navigates on item press and triggers haptic feedback", () => {
    ...
    const itemButton = getByLabelText("User Accounts");
  ...
  it("triggers onToggleCollapse when collapse button is pressed", () => {
    ...
    const { getByLabelText } = render(
      <AdminSidebar collapsed={false} onToggleCollapse={mockToggle} />
    );

    const collapseButton = getByLabelText("Collapse admin sidebar");
