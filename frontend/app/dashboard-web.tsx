/**
 * Admin Dashboard Web - Lavanya eMart
...
import { Alert, RefreshControl, ScrollView, Text, View, Platform } from "react-native";
...
const SafeAnimatedView: React.FC<SafeAnimatedViewProps> = ({
  ...
  const animationProps = entering
    ? { entering: entering.delay(delay).duration(400).springify() }
    : {};
  return (
...
export default function DashboardWeb() {
  ...
  const handleGenerateReport = async (reportType: string) => {
    if (offlineMode) {
      Alert.alert(
        "Offline Mode",
        "Report generation requires a live connection."
      );
      return;
    }
  ...
  const handleAutoFix = async (issue: any) => {
    if (offlineMode) {
      Alert.alert(
        "Offline Mode",
        "Auto-fix actions require a live connection."
      );
      return;
    }
