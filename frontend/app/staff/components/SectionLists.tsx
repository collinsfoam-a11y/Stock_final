...
const getSessionLabel = (session: SessionListItem) =>
  session.warehouse || session.session_id || session.id || "section";

const getItemCount = (session: SessionListItem) => session.item_count || session.total_items || 0;
      ...
      <View style={styles.section}>
        <View style={styles.sectionHeaderRow}>
          ...
          <TouchableOpacity
            ...
            accessibilityHint="Creates a new counting section"
          ...
          <View style={styles.listContainer}>
            {topActiveSections.map((session, index) => (
              <Animated.View
                ...
                entering={prefersReducedMotion ? undefined : FadeInUp.delay(100 + index * 80)}
              >
                <ModernCard
                  variant="elevated"
                  onPress={() =>
                  ...
                  accessibilityLabel={`Resume ${getSessionLabel(session)}, ${getItemCount(
                    session
                  )} items`}
                  accessibilityHint="Opens this section for scanning"
                >
                  <View
                    style={[
                      styles.sessionIcon,
              ...
              <>
                ...
                >
                  {overflowActiveSections.map((session, index) => (
                    <Animated.View
                      key={session.id || session.session_id}
                      entering={prefersReducedMotion ? undefined : FadeInUp.delay(300 + index * 60)}
      ...
      <View style={styles.section}>
        <View style={styles.sectionHeaderRow}>
          ...
          <TouchableOpacity
            ...
            accessibilityHint={
              showFinishedSearch ? "Hides previous session search" : "Shows previous session search"
            }
            accessibilityState={{ expanded: showFinishedSearch }}
            hitSlop={TOUCH_HIT_SLOP}
          >
            <Ionicons
              name="search"
          ...
          <View style={styles.listContainer}>
            {(showAllFinished ? finishedSections : finishedSections.slice(0, 3)).map(
              (session, index) => (
                <Animated.View
                  key={session.id || session.session_id}
                  entering={prefersReducedMotion ? undefined : FadeInUp.delay(200 + index * 50)}
                >
                  <View
                    style={[
            ...
            {finishedSections.length > 3 && (
              <TouchableOpacity
                ...
                accessibilityHint="Toggles the previous sessions list length"
                hitSlop={TOUCH_HIT_SLOP}
              >
