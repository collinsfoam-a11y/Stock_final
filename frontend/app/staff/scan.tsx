...
import { ExceptionCard } from "@/components/operational/ExceptionCard";
...
const ScanScreen = React.memo(function ScanScreen() {
  ...
  const [scanException, setScanException] = useState<ExceptionViewModel[] | null>(null);
  ...
  const handleFinishRack = async () => {
    ...
    try {
      ...
      const vm = toExceptionViewModel({
        ...
        message: error?.message || "This session could not be submitted for supervisor review.",
        description:
  ...
  return (
    ...
    >
      ...
      />
      {scanException?.map((vm, i) => (
        <View key={`${vm.code}-${i}`} style={styles.scanExceptionWrapper}>
          <ExceptionCard
            vm={vm}
            onAction={(actionVm) => {
              if (actionVm.action?.journey === "DISMISS") {
                setScanException((prev) => (prev ? prev.filter((v) => v.code !== actionVm.code) : []));
              }
            }}
          />
        </View>
