        ...
        for line in body:
            ...
            for imp in original_imports:
                if line.strip() == imp.strip():
                    ...
                    break
            if not is_dupe:
                # Also filter out my messy try: except: logic errors
                if 'try:' in line and 'import pyodbc' not in line and 'MappingPreviewResponse' not in line:
                    # check if next line is except
                    idx = body.index(line)
                    if idx + 1 < len(body) and 'except ' in body[idx+1]:
                        continue
