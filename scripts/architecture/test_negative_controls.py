...
def test_negative_controls(root_dir):
    ...
    try:
        ...
        res_circ = subprocess.run(f"{sys.executable} scripts/architecture/circular_dependency_check.py {temp_dir}", shell=True, capture_output=True, text=True)
        results.append({
            ...
            "status": "PASSED_NEGATIVE_CONTROL" if res_circ.returncode != 0 else "FAILED_TO_DETECT_BAD_FIXTURE"
        })

        # 2. Boundary Leakage Negative Control (Shared imports react-native)
        shared_dir = temp_dir / "frontend" / "packages" / "shared"
        shared_dir.mkdir(parents=True)
        (shared_dir / "leaky.ts").write_text("import { View } from 'react-native'; export const LeakyView = View;")

        res_leak = subprocess.run(f"{sys.executable} scripts/architecture/boundary_leakage_check.py {temp_dir}", shell=True, capture_output=True, text=True)
        results.append({
            ...
            "status": "PASSED_NEGATIVE_CONTROL" if res_leak.returncode != 0 else "FAILED_TO_DETECT_BAD_FIXTURE"
        })

        # 3. Secret Scanner Negative Control (Hardcoded Private Key)
        sec_dir = temp_dir / "secrets"
        sec_dir.mkdir()
        (sec_dir / "leaked.ts").write_text("const PRIVATE_KEY = '-----BEGIN RSA PRIVATE KEY-----\\nMIIEowIBAAKCAQEA...';")

        res_sec = subprocess.run(f"{sys.executable} scripts/architecture/secret_scan.py {temp_dir}", shell=True, capture_output=True, text=True)
        results.append({
            "scanner": "Secret Scanner",
            "fixture": "Hardcoded RSA Private Key",
            "expected_outcome": "FAIL (Real Secret Found)",
            "actual_returncode": res_sec.returncode,
            "status": "PASSED_NEGATIVE_CONTROL" if res_sec.returncode != 0 else "FAILED_TO_DETECT_BAD_FIXTURE"
        })
        ...
        results.append({
            ...
            "status": "PASSED_NEGATIVE_CONTROL" if not has_token_prefix_in_code else "FAILED_TOKEN_LOGGING_FOUND"
        })
...
if __name__ == '__main__':
    ...
    print(f"Architecture Negative Controls Suite: {rep['total_controls_tested']} controls tested. Status: {rep['status']}")
