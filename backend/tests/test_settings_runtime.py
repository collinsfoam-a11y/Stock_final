...
def test_run_server_main_loads_backend_env_before_validation(monkeypatch, tmp_path):
    logger = MagicMock()
    ...
    (backend_dir / ".env").write_text(
        "\n".join(
            [
                ...
                "PIN_SALT=cccccccccccccccccccccccccccccccc",
            ]
        ),
        encoding="utf-8",
    )

    for key in ("MONGO_URL", "JWT_SECRET", "JWT_REFRESH_SECRET", "PIN_SALT"):
        monkeypatch.delenv(key, raising=False)
    monkeypatch.setenv("DB_NAME", "from_process")
    ...
    def assert_env_loaded():
        ...
        assert os.getenv("MONGO_URL") == "mongodb://127.0.0.1:27017"
        assert os.getenv("DB_NAME") == "from_process"
        assert os.getenv("JWT_SECRET") == "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
        assert os.getenv("JWT_REFRESH_SECRET") == "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
        assert os.getenv("PIN_SALT") == "cccccccccccccccccccccccccccccccc"

    with (
        patch("backend.utils.env_validation.validate_environment", side_effect=assert_env_loaded),
