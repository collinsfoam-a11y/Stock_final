...
async def test_error_logging(mock_db, mock_supervisor):
    ...
    log_id = await service.log_error(
        error=error, error_type="ValueError", severity="error", endpoint="/test", method="GET", user="supervisor"
    )
    ...
    result = await service.get_recent_errors(severity="error", days_back=7)
...
async def test_separation_of_sql_and_mongo(mock_db):
    ...
    # Mock locks collection for sync run lock
    mock_locks = MagicMock()
    mock_locks.insert_one = AsyncMock(return_value=MagicMock(inserted_id="lock-1"))
    mock_db.locks = mock_locks
