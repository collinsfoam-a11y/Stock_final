...
def _make_mock_route(
    path: str, methods: set[str], name: str = "test", responses: dict | None = None
):
    return APIRoute(
        path=path,
...
class TestMobileCompatibility:
    ...
    def test_302_on_mutation_fails(self):
        app = FastAPI()
        route = _make_mock_route(
            "/api/sync/batch", {"POST"}, "sync_batch", responses={302: {"description": "Found"}}
        )
        app.routes.append(route)
...
class TestReplicaSetGuard:
    ...
    async def test_fails_in_production_with_standalone_mongo(self):
        ...
        mock_client = AsyncMock()
        mock_client.admin.command.side_effect = Exception("not running with --replSet")

        import backend.config.core as config_mod

        original = getattr(config_mod.settings, "ENVIRONMENT", "development")
        config_mod.settings.ENVIRONMENT = "production"  # type: ignore[attr-defined]
        try:
            with pytest.raises(RuntimeError, match="replica set"):
                await check_mongodb_replica_set(mock_client)
        finally:
            config_mod.settings.ENVIRONMENT = original  # type: ignore[attr-defined]
    ...
    async def test_warns_in_development_with_standalone_mongo(self):
        ...
        mock_client = AsyncMock()
        mock_client.admin.command.side_effect = Exception("no replset config")

        import backend.config.core as config_mod

        original = getattr(config_mod.settings, "ENVIRONMENT", "development")
        config_mod.settings.ENVIRONMENT = "development"  # type: ignore[attr-defined]
        try:
            result = await check_mongodb_replica_set(mock_client)
