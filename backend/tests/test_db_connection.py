...
class TestSQLServerConnectionBuilder:
    ...
    def test_build_connection_string_success(self, mock_pyodbc_drivers):
        ...
        assert "ApplicationIntent=ReadOnly" in conn_str
        assert "ApplicationIntent=ReadWrite" not in conn_str
    ...
    def test_build_connection_string_rejects_read_write_override(self, mock_pyodbc_drivers):
        mock_pyodbc_drivers.return_value = ["ODBC Driver 17 for SQL Server"]
        SQLServerConnectionBuilder._detected_driver = None

        with pytest.raises(ValueError, match="ApplicationIntent=ReadOnly"):
            SQLServerConnectionBuilder.build_connection_string(
                host="localhost",
                database="testdb",
                additional_params={"ApplicationIntent": "ReadWrite"},
            )
