...
class TestCheckSerialUniqueness:
    ...
    @pytest.mark.asyncio
    async def test_requires_item_code(self, mock_db):
        with patch("backend.api.count_lines_routes.get_db", return_value=mock_db):
...
class TestCreateCountLine:
    ...
    def mock_db(self):
        ...
        # find() is NOT awaited; its result is used in 'async for', so use Mock + AsyncIter
        db.count_lines.find = Mock(return_value=AsyncIter([]))
    ...
    async def test_create_count_line_duplicate(self, mock_db, line_data, erp_item):
        ...
        # find_duplicate_count_line uses find() with async for
        existing_line = {
            "id": "existing",
            "session_id": "session123",
            "item_code": "ITEM001",
    ...
    @pytest.mark.asyncio
    async def test_create_count_line_treats_none_snapshot_qty_as_zero(
        self, mock_db, line_data, erp_item
    ):
        ...
        assert result["erp_qty"] == 0.0
        assert result["variance"] == line_data.counted_qty
...
class TestApprovalWorkflow:
    ...
    @pytest.mark.asyncio
    async def test_delete_count_line_archives_instead_of_hard_deleting(self):
        mock_db = AsyncMock()
        count_line = {
            "_id": "mongo-id-archive",
            "id": "line-archive",
        ...
        write_service.process_write = AsyncMock(return_value=Mock(matched_count=1, modified_count=1))

        with (
            patch("backend.api.count_lines_routes._get_db_client", return_value=mock_db),
            patch("backend.api.count_lines_routes._ensure_count_line_mutable", new=AsyncMock()),
            patch(
            ...
            patch("backend.api.count_lines_routes._recalculate_session_stats", new=AsyncMock()),
            patch("backend.api.count_lines_routes._broadcast_dashboard_refresh", new=AsyncMock()),
            patch("backend.api.count_lines_routes._log_delete_activity", new=AsyncMock()),
        ):
            result = await delete_count_line(
                line_id="line-archive",
                request=AsyncMock(),
                current_user={"username": "supervisor", "role": "supervisor"},
            )

        assert result == {"success": True, "message": "Count line archived successfully"}
        payload = write_service.process_write.await_args.args[0]
        context = write_service.process_write.await_args.kwargs["context"]
        assert payload["operation"] == "update_one"
        assert payload["update"]["$set"]["archived"] is True
        assert payload["update"]["$set"]["status"] == "superseded"
        assert context["governance_mode"] == "mutable_session"
    ...
    async def test_reject_count_line_assigns_recount_and_notifies_both_users(self):
        mock_db = AsyncMock()
        mock_db.sessions.find_one = AsyncMock(return_value={"id": "session456", "status": "OPEN"})
        mock_db.count_lines.find_one = AsyncMock(
            return_value={
                "_id": "mongo-id-2",
                "id": "line456",
