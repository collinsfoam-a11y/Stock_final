    ...
    async def test_create_session_empty_warehouse(self, mock_user_staff):
        ...
        try:
            async with AsyncClient(
                ...
                assert response.json()["detail"] == "Invalid input. Please check your data and try again."
        finally:
            app.dependency_overrides.clear()
        ...
        try:
            ...
            with patch(
                ...
                async with AsyncClient(
                    ...
                    data = response.json()
                    assert data["detail"] == "An unexpected error occurred. Please try again or contact support."
        finally:
            app.dependency_overrides.clear()
    ...
    async def test_complete_session_not_owner(self, mock_user_staff):
        ...
        try:
            async with AsyncClient(
                ...
                assert response.status_code == 410
                assert response.json()["detail"] == "An unexpected error occurred. Please try again or contact support."
        finally:
            app.dependency_overrides.clear()
