...
async def test_legacy_sync_replay_is_rejected(async_client, test_db, authenticated_headers):
    ...
    assert first.json()["detail"] == "An unexpected error occurred. Please try again or contact support."
    assert second.json()["detail"] == "An unexpected error occurred. Please try again or contact support."
