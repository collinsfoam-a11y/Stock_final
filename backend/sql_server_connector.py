...
DISALLOWED_ADVANCED_SQL_KEYWORDS = (
    "OPENQUERY",
    "OPENROWSET",
    "OPENDATASOURCE",
    "WAITFOR",
...
DISALLOWED_PROCEDURE_PATTERN = re.compile(r"\b(?:XP_[A-Z0-9_]*|SP_OA[A-Z0-9_]*)\b")
SQL_IDENTIFIER_PART = r'(?:\[[^\]]+\]|"(?:""|[^"])+"|[A-Z_][A-Z0-9_$#@]*)'
LINKED_SERVER_PATTERN = re.compile(
    rf"{SQL_IDENTIFIER_PART}\s*\.\s*{SQL_IDENTIFIER_PART}\s*\.\s*"
    rf"{SQL_IDENTIFIER_PART}\s*\.\s*{SQL_IDENTIFIER_PART}",
    flags=re.IGNORECASE,
)
...
class SQLServerConnector:
    ...
    def _build_sales_metadata(self):
        ...
        ref_price = self._get_column_reference("S", cols, ["SalePrice", "MRP", "Price"])
        if ref_price:
            return f", {ref_price} AS SalesPrice"
        return ""
    ...
    def _build_brand_metadata(self):
        ...
        name_ref = self._get_column_reference("B", cols, ["BrandName", "Name", "Description"])
        if name_ref:
            return f" LEFT JOIN {t_brand} B ON P.BrandID = B.BrandID", f", {name_ref} AS BrandName"
        return "", ""
    ...
    def _build_purchase_metadata(self):
        ...
        ref_rate = self._get_column_reference("PUR", cols, ["PurchaseRate", "Cost", "Rate"])
        if ref_rate:
            return f", {ref_rate} AS PurchaseRate"
        return ""
    ...
    def _strip_sql_comments_and_string_literals(self, query: str) -> str:
        """Remove comments and strings while retaining identifier structure."""
    ...
    def _strip_sql_comments_and_literals(self, query: str) -> str:
        ...
        sanitized = self._strip_sql_comments_and_string_literals(query)
    ...
    def _assert_select_only_prefix(self, normalized_query: str) -> None:
        if normalized_query.startswith(("SELECT ", "WITH ")):
            return
    ...
    def _assert_read_only_query(self, query: str) -> None:
        ...
        self._assert_no_disallowed_tokens(
            normalized,
            DISALLOWED_ADVANCED_SQL_KEYWORDS,
            "ERP read-only violation: advanced SQL construct blocked",
        )

        if DISALLOWED_PROCEDURE_PATTERN.search(normalized):
            raise ERPReadOnlyViolation("ERP read-only violation: system procedure blocked")

        structural_query = self._strip_sql_comments_and_string_literals(query)
        if LINKED_SERVER_PATTERN.search(structural_query):
            raise ERPReadOnlyViolation("ERP read-only violation: linked-server reference blocked")
    ...
    def is_safe_query(self, query: str, params: Sequence[Any] | None = None) -> bool:
        """Compatibility wrapper exposing the runtime policy without executing SQL."""
        try:
            self._assert_read_only_query(query)
            self._validate_params(query, params)
        except (DatabaseQueryError, TypeError, ValueError):
            return False
        return True
    ...
    def get_item_quantities_only(self, item_codes: list[str]) -> dict[str, float]:
        ...
        try:
            ...
            raise DatabaseQueryError(
                f"Failed to fetch item quantities by item code: {e!s}"
            ) from e
    ...
    async def execute_query(
        ...
        if not self.connection and not self._attempt_auto_reconnect():
            raise DatabaseConnectionError(DB_NOT_CONNECTED_MSG)
    ...
    def _is_read_only_query(cls, query: str) -> bool:
        ...
        return upper.startswith(("SELECT", "WITH"))
