...
class Settings(PydanticBaseSettings):
    ...
    MONGO_MAX_POOL_SIZE: int = Field(60, ge=1)
    MONGO_MIN_POOL_SIZE: int = Field(0, ge=0)
    MONGO_MAX_IDLE_TIME_MS: int = Field(300000, ge=1000)
    MONGO_SERVER_SELECTION_TIMEOUT_MS: int = Field(5000, ge=1000)
    MONGO_CONNECT_TIMEOUT_MS: int = Field(10000, ge=1000)
    MONGO_SOCKET_TIMEOUT_MS: int = Field(30000, ge=1000)
    MONGO_RETRY_WRITES: bool = True
    MONGO_RETRY_READS: bool = True
    MONGO_REQUIRE_TRANSACTIONS: Optional[bool] = None
    ...
    PI_SERVER_TOKEN: Optional[str] = Field(
        default=None, description="Bearer token for the pi-server sidecar"
    )
...
try:
    ...
    class FallbackSettings:
        def __init__(self):
            ...
            self.MONGO_MAX_POOL_SIZE = int(os.getenv("MONGO_MAX_POOL_SIZE", 60))
            self.MONGO_MIN_POOL_SIZE = int(os.getenv("MONGO_MIN_POOL_SIZE", 0))
            self.MONGO_MAX_IDLE_TIME_MS = int(os.getenv("MONGO_MAX_IDLE_TIME_MS", 300000))
            self.MONGO_SERVER_SELECTION_TIMEOUT_MS = int(
            ...
            self.MONGO_SOCKET_TIMEOUT_MS = int(os.getenv("MONGO_SOCKET_TIMEOUT_MS", 30000))
            self.MONGO_RETRY_WRITES = (
                os.getenv("MONGO_RETRY_WRITES", "true").lower() == "true"
            )
            self.MONGO_RETRY_READS = os.getenv("MONGO_RETRY_READS", "true").lower() == "true"
            raw_require_tx = os.getenv("MONGO_REQUIRE_TRANSACTIONS")
            self.MONGO_REQUIRE_TRANSACTIONS = (
                None if raw_require_tx is None else _parse_bool(raw_require_tx, default=False)
            )
            ...
            self.PI_SERVER_TOKEN = _secret_env_first("PI_SERVER_TOKEN")
