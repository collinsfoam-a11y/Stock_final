...
async def check_auth_rate_limits(request: Request, identifier: str | None = None) -> None:
...
async def record_auth_failure(request: Request, identifier: str | None = None) -> None:
    if not getattr(settings, "RATE_LIMIT_ENABLED", True):
        return
...
async def reset_auth_limits(request: Request, identifier: str | None = None) -> None:
    ip_key = get_rate_limit_key(request)
    user_key = get_rate_limit_key(request, identifier) if identifier else None
