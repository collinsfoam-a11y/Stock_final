...
class SessionLifecycleService:
    ...
    async def assess_finalization_readiness(
        self,
        session_id: str,
        *,
        db_session: Any | None = None,
        ...
        """
        FIX GROUP 10: Read-only pre-finalization assessment.

        Returns structured blocker data (never raises) so the API layer can
        present a backend-driven checklist to supervisors before they attempt
        finalization.
        """
        kwargs = self._kwargs(db_session)
        blockers: list[dict[str, Any]] = []

        # 1. Unresolved unknown items (canonical code: UNKNOWN_ITEMS_PENDING)
        unknown_count = await self.db.unknown_items.count_documents(
            {"session_id": session_id, "status": {"$nin": ["RESOLVED", "DISMISSED"]}},
            **kwargs,
        )
        if unknown_count > 0:
            blockers.append({
                ...
                "description": f"{unknown_count} unresolved unknown item(s)",
                "severity": "blocking",
            })

        # 2. Pending recount requests (canonical code: RECOUNT_REQUESTS_PENDING)
        recount_count = await self.db.recount_requests.count_documents(
            {
                "session_id": session_id,
                "status": {"$nin": ["completed", "cancelled", "expired"]},
        ...
        if recount_count > 0:
            blockers.append({
                "code": "RECOUNT_REQUESTS_PENDING",
                "canonical_code": "FI-02",
                "description": f"{recount_count} pending recount request(s)",
                "severity": "blocking",
            })

        # 3. Count lines requiring approval (canonical code: COUNT_LINES_PENDING_APPROVAL)
        needs_review_count = await self.db.count_lines.count_documents(
            {
                "session_id": session_id,
                "approval_status": {"$in": ["NEEDS_REVIEW", "PENDING"]},
        ...
        if needs_review_count > 0:
            blockers.append({
                "code": "COUNT_LINES_PENDING_APPROVAL",
                "canonical_code": "R9.1",
                "description": f"{needs_review_count} unresolved count lines pending approval",
                "severity": "blocking",
            })
