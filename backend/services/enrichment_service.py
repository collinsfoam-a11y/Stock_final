...
class EnrichmentService:
    ...
    async def record_enrichment(
        ...
        try:
            ...
            completeness = await self.calculate_completeness(item_code, update_fields, prefetched_item=existing_item)
    ...
    async def calculate_completeness(
        self, item_code: str, additional_fields: dict[str, Optional[Any]] = None, prefetched_item: dict[str, Any] = None
    ) -> dict[str, Any]:
        """
        ...
        Args:
            ...
            prefetched_item: Pre-fetched item to avoid N+1 DB lookup

        Returns:
            Dictionary with completeness information
        """
        # Get current item if not prefetched
        item = dict(prefetched_item) if prefetched_item else await self.db.erp_items.find_one({"item_code": item_code})
        if not item:
            return {
                "is_complete": False,
                "percentage": 0,
