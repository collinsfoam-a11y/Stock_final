    ...
    async def _map_unknown_to_known_item(
        ...
        *,
        ...
        await self.audit_service.log_write_event(
            ...
            session_id=session_id,
            item_id=str(new_count_line.get("item_code") or "") or None,
            location_id=location_id,
            actor_id=actor_id,
            version=1,
            idempotency_key=str(new_count_line.get("idempotency_key") or "") or None,
            semantic_hash=str(new_count_line.get("semantic_hash") or "") or None,
            metadata={
    ...
    async def dismiss_unknown_item(
        ...
        *,
        ...
        async with MongoUnitOfWork(self.db.client) as uow:
            ...
            if session_id:
                await self.lifecycle_service.ensure_session_not_finalized(
                    session_id, db_session=uow.session
                )
            ...
            await self.audit_service.log_write_event(
                ...
                location_id=str(resolved_doc.get("location_id") or "") or None,
                actor_id=actor_id,
