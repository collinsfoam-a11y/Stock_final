...
class DataGovernanceService:
    ...
    async def process_erasure_request(
        ...
        # Collections to process. Stock count records are anonymized below instead
        # of deleted so inventory truth and projections remain auditable.
        default_collections = ["activity_logs"]
        collections = collections_to_erase or default_collections

        results = {}

        if collections_to_erase is None or "count_lines" in collections_to_erase:
            results["count_lines_anonymized"] = await self._anonymize_count_line_actor_fields(
                subject_id=subject_id,
                processed_by=processed_by,
                request_id=request_id,
            )
            collections = [name for name in collections if name != "count_lines"]
    ...
    async def _anonymize_count_line_actor_fields(
        self,
        *,
        subject_id: str,
        processed_by: str,
        ...
        actor_filter = {"$or": [{field: subject} for field in COUNT_LINE_ACTOR_FIELDS]}
        anonymized_actor = f"deleted_user_{subject[:8]}"
        now = datetime.now(timezone.utc).replace(tzinfo=None)
        candidate_lines = await self.db.count_lines.find(actor_filter).limit(5000).to_list(5000)
        if not candidate_lines:
            return 0

        set_doc: dict[str, Any] = {
            "privacy_erased_at": now,
            "privacy_erasure_request_id": request_id,
            "privacy_erased_by": processed_by,
            "updated_at": now,
            "updated_by": processed_by,
        }
        for field in COUNT_LINE_ACTOR_FIELDS:
            set_doc[field] = anonymized_actor if field != "assigned_to" else None

        result = await CountLineWriteService(self.db).process_write(
            {
