...
class SystemReportService:
    ...
    async def _count_rows_in_range(
        ...
        # Optimization: use native count_documents query instead of pulling all full
        # documents into application memory and filtering them via python loops.
        # This matches the behavior of fetching through _fetch_collection_documents.
        query: dict[str, Any] = {}
        ...
        # Ensure timestamps aren't missing.
        if timestamp_field not in query:
            query[timestamp_field] = {}
        query[timestamp_field]["$exists"] = True
        query[timestamp_field]["$ne"] = None
