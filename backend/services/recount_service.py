...
class RecountService:
    ...
    async def assign(self, db, request_id: str, assigned_to: str) -> RecountRequest | None:
    ...
    async def start(self, db, request_id: str) -> RecountRequest | None:
        result = await db["recount_requests"].find_one_and_update(
            {"id": request_id},
            {
                "$set": {
                    "started_at": _utc_now(),
                    "status": CountObservationStatus.RECOUNT_IN_PROGRESS.value,
                    "updated_at": _utc_now(),
                }
            },
            return_document=True,
    ...
    async def submit_recount(
        ...
        if difference == 0 and matches_sql or matches_sql:
