...
class ApprovalEngine:
    ...
    async def evaluate(
        ...
        if not sql_qty and sql_source != SqlAvailability.AVAILABLE_AT_SYNC:
            rules.append(
                {
                    "rule_name": "sql_fetched_at_submission",
                    "passed": False,
                    "detail": "SQL unavailable at submission",
                }
            )
            return AutoApprovalResult(
        ...
        rules.append(
            {"rule_name": "sql_fetched_at_submission", "passed": True, "detail": str(sql_qty)}
        )

        batch_ok = self._check_batch_variance(observation, db)
        rules.append(
            {"rule_name": "batch_variance", "passed": batch_ok.passed, "detail": batch_ok.detail}
        )
        if not batch_ok.passed:
            exceptions.append(
                self._exception(ApprovalExceptionType.BATCH_VARIANCE, batch_ok.detail)
            )

        serial_ok = self._check_serial_integrity(observation, db)
        rules.append(
            {
                "rule_name": "serial_integrity",
                "passed": serial_ok.passed,
                "detail": serial_ok.detail,
            }
        )
        if not serial_ok.passed:
            exceptions.append(
                self._exception(ApprovalExceptionType.SERIAL_CONFLICT, serial_ok.detail)
            )

        attr_ok = self._check_attribute_mismatch(observation)
        rules.append(
            {"rule_name": "attribute_match", "passed": attr_ok.passed, "detail": attr_ok.detail}
        )
        if not attr_ok.passed:
            exceptions.append(
                self._exception(ApprovalExceptionType.ATTRIBUTE_MISMATCH, attr_ok.detail)
            )

        location_ok = self._check_location_policy(observation)
        rules.append(
            {
                "rule_name": "location_policy",
                "passed": location_ok.passed,
                "detail": location_ok.detail,
            }
        )
        if not location_ok.passed:
            exceptions.append(
                self._exception(ApprovalExceptionType.LOCATION_MISMATCH, location_ok.detail)
            )

        damage_ok = self._check_damaged_stock(observation)
        rules.append(
            {"rule_name": "damaged_stock", "passed": damage_ok.passed, "detail": damage_ok.detail}
        )
        if not damage_ok.passed:
            exceptions.append(
                self._exception(ApprovalExceptionType.DAMAGED_STOCK, damage_ok.detail)
            )

        provisional_ok = self._check_provisional(observation)
        rules.append(
            {
                "rule_name": "provisional_items",
                "passed": provisional_ok.passed,
                "detail": provisional_ok.detail,
            }
        )
        if not provisional_ok.passed:
            exceptions.append(
                self._exception(ApprovalExceptionType.PROVISIONAL_BATCH, provisional_ok.detail)
            )

        additional_ok = await self._check_additional_location_investigation(observation, db)
        rules.append(
            {
                "rule_name": "additional_location",
                "passed": additional_ok.passed,
                "detail": additional_ok.detail,
            }
        )
        if not additional_ok.passed:
            exceptions.append(
                self._exception(ApprovalExceptionType.LOCATION_INVESTIGATION, additional_ok.detail)
            )

        remark_ok = self._check_mandatory_remark(observation)
        rules.append(
            {
                "rule_name": "mandatory_remark",
                "passed": remark_ok.passed,
                "detail": remark_ok.detail,
            }
        )
        if not remark_ok.passed:
            exceptions.append(
                self._exception(ApprovalExceptionType.INCOMPLETE_EVIDENCE, remark_ok.detail)
            )

        param_ok = self._check_required_parameters(observation)
        rules.append(
            {
                "rule_name": "required_parameters",
                "passed": param_ok.passed,
                "detail": param_ok.detail,
            }
        )
        if not param_ok.passed:
            exceptions.append(
                self._exception(ApprovalExceptionType.ATTRIBUTE_MISMATCH, param_ok.detail)
            )

        accessory_ok = self._check_accessory_completeness(observation)
        rules.append(
            {
                "rule_name": "accessory_checks",
                "passed": accessory_ok.passed,
                "detail": accessory_ok.detail,
            }
        )
        if not accessory_ok.passed:
            exceptions.append(
                self._exception(ApprovalExceptionType.INCOMPLETE_EVIDENCE, accessory_ok.detail)
            )

        photo_ok = self._check_photo_evidence(observation, db)
        rules.append(
            {"rule_name": "photo_evidence", "passed": photo_ok.passed, "detail": photo_ok.detail}
        )
        if not photo_ok.passed:
            exceptions.append(
                self._exception(ApprovalExceptionType.INCOMPLETE_EVIDENCE, photo_ok.detail)
            )

        conf_ok = await self._check_concurrent_conflict(observation, db)
        rules.append(
            {"rule_name": "concurrent_conflict", "passed": conf_ok.passed, "detail": conf_ok.detail}
        )
        if not conf_ok.passed:
            exceptions.append(
                self._exception(ApprovalExceptionType.CONCURRENT_OBSERVATION, conf_ok.detail)
            )

        sync_ok = self._check_sync_conflict(observation)
        rules.append(
            {"rule_name": "sync_conflict", "passed": sync_ok.passed, "detail": sync_ok.detail}
        )
        ...
        rules.append(
            {
                "rule_name": "policy_thresholds",
                "passed": threshold_ok.passed,
                "detail": threshold_ok.detail,
            }
        )
        if not threshold_ok.passed:
            exceptions.append(
                self._exception(ApprovalExceptionType.POLICY_THRESHOLD, threshold_ok.detail)
            )

        approved = len(exceptions) == 0 and variance == 0.0
        system_recommendation = (
            SystemRecommendation.AUTO_APPROVE
            if approved
            else SystemRecommendation.SUPERVISOR_REVIEW
        )
        ...
        return AutoApprovalResult(
            ...
            rules_passed=rules,
            sql_quantity_source=sql_source,
            sql_comparison_source=sql_source.value
            if sql_source.value != SqlAvailability.UNAVAILABLE.value
            else None,
            block_reason="; ".join(e.description or e.exception_type.value for e in exceptions)
            if not approved
            else None,
        )
    ...
    def _exception(
        self, exception_type: ApprovalExceptionType, detail: str | None = None
    ) -> ApprovalExceptionDetail:
        return ApprovalExceptionDetail(
            exception_type=exception_type, description=detail, is_blocking=True
        )
    ...
    def _check_location_policy(self, observation: dict[str, Any]) -> RuleResult:
        ...
        actual = (
            observation.get("location_id")
            or observation.get("mark_location")
            or observation.get("rack_no")
        )
    ...
    def _check_damaged_stock(self, observation: dict[str, Any]) -> RuleResult:
        ...
        if damaged > 0 or non_returnable > 0 or condition:
            return self._fail(
                "damaged_or_condition",
                f"damaged={damaged}, nr={non_returnable}, condition={condition}",
            )
        return self._ok("no_damage")

    def _check_provisional(self, observation: dict[str, Any]) -> RuleResult:
        is_provisional = (
            observation.get("is_provisional_batch")
            or observation.get("is_provisional_bundle")
            or observation.get("is_new_bundle")
        )
        ...
        cursor = coll.find(
            {
                ...
                "response": AdditionalLocationResponse.NOT_CHECKED.value,
            }
        )
        ...
        cursor = coll.find(
            {
                ...
                "status": {
                    "$nin": [
                        CountObservationStatus.APPROVED.value,
                        CountObservationStatus.AUTO_APPROVED.value,
                    ]
                },
            }
        )
    ...
    def _fail(self, detail: str, extra: str | None = None) -> RuleResult:
        return RuleResult(passed=False, detail=f"{detail}:{extra}" if extra else detail)


class RuleResult:
    def __init__(self, passed: bool, detail: str | None = None):
        self.passed = passed
        self.detail = detail
