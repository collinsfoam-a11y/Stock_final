...
MAX_ACTIVE_COUNT_LINES_PER_ITEM = 500
...
class InventoryReadRouter:
    ...
    async def _active_count_lines(
        ...
        *,
        ...
        cursor = self.db.count_lines.find({"session_id": session_id, "item_code": item_code}).limit(
            MAX_ACTIVE_COUNT_LINES_PER_ITEM
        )
        count_lines = await cursor.to_list(MAX_ACTIVE_COUNT_LINES_PER_ITEM)
