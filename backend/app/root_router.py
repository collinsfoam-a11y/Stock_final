...
async def get_sessions_analytics(current_user: dict = Depends(get_current_user)):
    if current_user["role"] not in ["supervisor", "admin"]:
        raise HTTPException(status_code=403, detail="Insufficient permissions")
    try:
        pipeline = [{"$group": {"_id": None, "total_sessions": {"$sum": 1}, "total_items": {"$sum": "$total_items"}, "total_variance": {"$sum": "$total_variance"}, "avg_variance": {"$avg": "$total_variance"}}}]
        status_pipeline = [{"$group": {"_id": "$status", "count": {"$sum": 1}}}]
        date_pipeline = [{"$project": {"date": {"$dateToString": {"format": "%Y-%m-%d", "date": "$started_at", "onNull": "unknown"}}, "warehouse": 1, "staff_name": 1, "total_items": 1, "total_variance": 1}}, {"$group": {"_id": "$date", "count": {"$sum": 1}}}, {"$sort": {"_id": 1}}]
        ...
        # ⚡ Bolt: Execute independent MongoDB aggregation pipelines concurrently to reduce sequential I/O bottlenecks.
        # Expected Impact: Reduces database query latency by roughly a factor of the number of pipelines (approx. ~5x speedup for this block).
        # Measurement: Check API response time for /legacy/sessions/analytics before and after.
        ...
        return {"success": True, "data": {"overall": overall_doc, "sessions_by_date": sessions_by_date, "sessions_by_status": sessions_by_status, "variance_by_warehouse": variance_by_warehouse, "items_by_staff": items_by_staff, "total_sessions": overall_doc.get("total_sessions", 0)}}
    except Exception as e:
...
async def get_session_by_id(session_id: str, current_user: dict = Depends(get_current_user)):
    try:
        ...
        if current_user["role"] not in ("supervisor", "admin") and session.get("staff_user") != current_user["username"]:
        ...
        logger.error("Error fetching session %s: %s", sanitize_for_logging(session_id), sanitize_for_logging(str(e), 200))
        raise HTTPException(status_code=500, detail=str(e)) from e
...
async def verify_stock(line_id: str, current_user: dict, *, request: Optional[Request] = None, db_override=None):
    ...
    update_result = await write_service.process_write(
        ...
        context={"transition": "verify", "username": current_user["username"], "session_id": str(count_line.get("session_id") or ""), "governance_mode": "mutable_session", "skip_session_totals_update": True}
    )
    ...
    if activity_log_service:
        await activity_log_service.log_activity(
            user=current_user["username"], role=current_user.get("role", ""), action="verify_stock", entity_type="count_line", entity_id=line_id, ip_address=request.client.host if request and request.client else None, user_agent=request.headers.get("user-agent") if request else None,
        )

    return {"message": "Stock verified", "verified": True}

async def unverify_stock(line_id: str, current_user: dict, *, request: Optional[Request] = None, db_override=None):
    _require_supervisor(current_user)
    db_client = _get_db_client(db_override)
    count_line = await db_client.count_lines.find_one({"id": line_id})
    if not count_line:
    ...
    update_result = await write_service.process_write(
        {"operation": "update_one", "filter": filter_query, "update": {"$set": {}}},
        context={"transition": "unverify", "username": current_user["username"], "session_id": str(count_line.get("session_id") or ""), "governance_mode": "mutable_session", "skip_session_totals_update": True}
    )
    if update_result.modified_count == 0:
        raise HTTPException(status_code=404, detail="Count line not found")
    ...
    if activity_log_service:
        await activity_log_service.log_activity(
            user=current_user["username"], role=current_user.get("role", ""), action="unverify_stock", entity_type="count_line", entity_id=line_id, ip_address=request.client.host if request and request.client else None, user_agent=request.headers.get("user-agent") if request else None,
        )
    return {"message": "Stock verification removed", "verified": False}

async def get_count_lines(session_id: str, current_user: dict, page: int = 1, page_size: int = 50, verified: Optional[bool] = None, *, db_override=None):
    ...
    lines_cursor = db_client.count_lines.find(filter_query, {"_id": 0}).sort("counted_at", -1).skip(skip).limit(page_size)
    lines = await lines_cursor.to_list(page_size)
    return {"items": lines, "pagination": {"page": page, "page_size": page_size, "total": total, "total_pages": (total + page_size - 1) // page_size, "has_next": skip + page_size < total, "has_prev": page > 1}}
