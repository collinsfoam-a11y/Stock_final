...
class RouterRegistry:
    ...
    approval_router: APIRouter | None = None
...
def _register_core_router_set(app: FastAPI, registry: RouterRegistry) -> None:
    specs: list[tuple[APIRouter, str | None, list[str] | None]] = [
        # Health router is registered once (not double-registered).
        # Both /health/* and /api/health/* paths are defined via stacked
        # decorators on the same handler in backend/api/health.py.
        # /health/* serves Docker production healthchecks; /api/health/*
        # serves API clients. Neither is a mutation endpoint, so §4.6
        # 301/302 mobile compatibility rules do not apply.
...
def _register_optional_router_set(app: FastAPI, registry: RouterRegistry, logger: Any) -> None:
    ...
    _include_optional_router(
        app,
        registry.approval_router,
        logger,
        success_log="Approval API router registered",
        failure_log="Approval router registration failed",
    )
    _include_optional_router(
        app,
        registry.damage_router,
        logger,
        success_log="Damage API router registered",
        failure_log="Damage router registration failed",
    )
