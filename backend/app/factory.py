...
from backend.app.root_router import api_router as legacy_root_api_router
from backend.app.root_router import root_router
...
def _sanitize_detail(status_code: int, detail: str) -> str:
    ...
    error_info = get_error_by_code(status_code)
    return error_info.get("message", "An error occurred. Please try again later.")

def _sanitize_error_response(status_code: int, detail: str) -> dict:
    """Generate a sanitized error response that doesn't expose sensitive details."""
    error_info = get_error_by_code(status_code)
    return {
        "detail": error_info.get("message", "An error occurred. Please try again later."),
        "code": error_info.get("code", "UNK_001"),
...
async def _http_exception_handler(request: Request, exc: StarletteHTTPException) -> JSONResponse:
    ...
    # Always sanitize 5xx errors to prevent exposing internal details
    if exc.status_code >= 500:
        response_data = _sanitize_error_response(exc.status_code, str(exc.detail))
    elif isinstance(exc.detail, dict):
        # For 4xx errors with dict details, we may allow more specific messages
        # but still sanitize sensitive fields
        if exc.status_code < 500:
            response_data = {"detail": exc.detail}
        else:
            response_data = _sanitize_error_response(exc.status_code, str(exc.detail))
    else:
        # For string details, sanitize appropriately
        response_data = _sanitize_error_response(exc.status_code, str(exc.detail))
...
async def _validation_exception_handler(
    ...
    # Return generic message to prevent revealing validation details
    return JSONResponse(
        status_code=422,
...
async def _generic_exception_handler(request: Request, exc: Exception) -> JSONResponse:
    logger.error(
        ...
        request.method,
        request.url.path,
        type(exc).__name__,  # Still log full details internally for debugging
    )
    # Always return sanitized response for 500 errors
    error_info = get_error_by_code(500)
    return JSONResponse(
        status_code=500,
        content={
            "detail": error_info.get("message", "The server encountered an unexpected error. Please try again later."),
            "code": error_info.get("code", "UNK_001"),
            "category": error_info.get("category", "unknown")
        },
    )
...
def create_app() -> FastAPI:
    ...
    _health_routes = [
        getattr(r, "path", "") for r in app.routes if "health" in getattr(r, "path", "").lower()
    ]
