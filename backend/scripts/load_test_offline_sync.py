...
class ConcurrentUserStressTester:
        ...
        try:
            ...
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    error_msg = f"Login failed: {response.status} {await response.text()}"
    ...
    async def _run_user_session(self, user: UserSession, http_session: aiohttp.ClientSession):
        ...
        while not self._stop_event.is_set():
            try:
                ...
                # Choose a random operation heavily weighted towards offline sync
    ...
    async def _offline_sync(self, http_session: aiohttp.ClientSession, user: UserSession) -> bool:
        """Perform an offline sync batch payload."""
        import uuid
        import datetime
        
        if not hasattr(user, 'active_session_id'):
            success, data = await self._make_authenticated_request(
                http_session, user, "GET", "/api/sessions"
            )
            if success and isinstance(data, list) and len(data) > 0:
                user.active_session_id = data[0].get("id") or data[0].get("session_id")
            else:
                return False

        if not getattr(user, 'active_session_id', None):
            return False

        client_record_id = str(uuid.uuid4())
        item_code = f"51{random.randint(1000, 9999)}"
            ...
            "records": [
                {
                    ...
                    "location_id": "LOC-1",
                    "floor_id": "FL-1",
                    "rack_id": f"RK-{random.randint(1, 100)}",
                    "item_code": item_code,
                    "verified_qty": random.randint(1, 10),
                    "damaged_qty": 0,
                    "serial_numbers": [],
                    "status": "finalized",
                    "created_at": now_iso,
    ...
    def print_results(self):
        ...
        if success_rate >= 99.0 and stats["errors"]["session_isolation_errors"] == 0:
            ...
            print("\n STRESS TEST: PASSED WITH WARNINGS")
        else:
            print("\n STRESS TEST: FAILED")
