...
def redact_connection_uri(uri: str | None) -> str:
    """Redact credentials from connection URIs before logging."""
    if not uri:
        return ""
    try:
        parsed = urlsplit(str(uri))
    except Exception:
        return sanitize_for_logging(str(uri))

    if not parsed.netloc or "@" not in parsed.netloc:
        return sanitize_for_logging(str(uri), max_length=120)

    _userinfo, hostinfo = parsed.netloc.rsplit("@", 1)
    redacted = urlunsplit(
        (
            parsed.scheme,
            f"***:***@{hostinfo}",
    ...
    return sanitize_for_logging(redacted, max_length=120)
