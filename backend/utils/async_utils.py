...
class AsyncExecutor:
    ...
    def _is_idempotent_operation(self, operation_name: str) -> bool:
        """
        Determine if an operation is idempotent and safe to retry.
        
        Non-idempotent operations that should NOT be retried:
        - payments
        - inserts
        - stock adjustments
        - notifications
        ...
        # Operations that are NOT safe to retry
        non_idempotent_patterns = [
            'payment', 'pay', 'charge', 'purchase', 'order', 'transaction',
            'insert', 'create', 'add', 'new', 'stock.adjust', 'adjustment',
            'notification', 'notify', 'send', 'erp.write', 'write', 
            'update.stock', 'inventory.adjust', 'count.submit', 
            'sync.submit', 'batch.submit'
        ]
    ...
    async def execute_with_retry(
        ...
        Retries are only performed for idempotent operations.
        ...
        # Determine if this operation is safe to retry
        is_idempotent = self._is_idempotent_operation(operation_name)
        max_attempts = self.retry_attempts if is_idempotent else 1
        ...
        for attempt in range(max_attempts):
            try:
                ...
                # Log the timeout but don't retry non-idempotent operations
                if not is_idempotent:
                    logger.warning(f"{operation_name} timed out (not retried due to non-idempotent nature): timeout")
                    break
                else:
                ...
                # Log the error but don't retry non-idempotent operations
                if not is_idempotent:
                    logger.warning(f"{operation_name} failed (not retried due to non-idempotent nature): {e!s}")
                    break
                else:
            ...
            # Exponential backoff for idempotent operations only
            if is_idempotent and attempt < max_attempts - 1:
                wait_time = self.backoff_factor**attempt
                await asyncio.sleep(wait_time)

        # All retries (if any) failed
        return Result.error(
    ...
    def _is_circuit_open(self, operation_name: str) -> bool:
        ...
        if state["failures"] >= self._circuit_breaker_threshold:
            ...
            if time_diff.total_seconds() > self._circuit_breaker_timeout:
...
def cached_async(key_func: Callable[..., str | None] | None = None, ttl: int = 1800):
    """
