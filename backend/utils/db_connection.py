...
class SQLServerConnectionBuilder:
    ...
    def build_connection_string(
        ...
        additional_params: dict[str, str | None] | None = None,
    ) -> str:
        """
        ...
        if environment in ["production", "staging"]:
            # For production, enforce encryption and certificate validation
            base_params.extend([
                "Encrypt=yes",
                "TrustServerCertificate=no",  # Important: validate certificates in production
            ])
        else:
            # For development, allow encrypted connections but trust server cert for convenience
            base_params.extend([
                "Encrypt=yes",  # Always use encryption
                "TrustServerCertificate=yes",  # Accept dev/self-signed certs
            ])
        ...
        if "ODBC Driver" in driver:
            base_params.extend(
                [
                    ...
                    # Every caller uses this builder for the governed read-only ERP.
                    # Request read-only routing when SQL Server AlwaysOn is available.
        ...
        if additional_params:
            for key, value in additional_params.items():
                if key.casefold() == "applicationintent" and str(value).casefold() != "readonly":
                    raise ValueError("ERP connections must use ApplicationIntent=ReadOnly")
