...
def _get_optional_db() -> Optional[Any]:
    """Return the active DB when available without blocking sidecar chat."""
    try:
        return get_db()
    except RuntimeError as e:
        logger.warning("AI Assistant DB context unavailable: %s", sanitize_for_logging(str(e)))
        return None


async def get_system_stats_context(db: Optional[Any]) -> str:
    ...
    try:
        ...
        accurate_scans = stats[0].get("accurate", 0) if stats else 0
        accuracy = (accurate_scans / total_scans * 100) if total_scans > 0 else 100
...
async def chat_with_pi(request: Request, current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    if not settings.PI_SERVER_TOKEN:
        raise HTTPException(
            status_code=503,
            detail="AI Assistant sidecar token is not configured.",
        )
        ...
        try:
            response = await client.post(
                ...
                headers={
                    ...
                    "Authorization": f"Bearer {settings.PI_SERVER_TOKEN}",
                },
            )
            ...
            try:
                ...
                if db is not None and user_msg and assistant_completion:
