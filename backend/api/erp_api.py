...
from backend.services.item_master_lookup import find_item_master
...
def _manual_items_collection() -> Any:
    if _db is None:
        return None
    collection = getattr(_db, "manual_items", None)
...
async def _count_items_across_catalogs(query: dict[str, Any]) -> int:
    if _db is None:
        return 0
    total = await _db.erp_items.count_documents(query)
    manual_items = _manual_items_collection()
    if manual_items is not None:
        total += await manual_items.count_documents(query)
    return total


async def _find_items_across_catalogs(
    query: dict[str, Any],
    *,
    skip: int,
    limit: int,
    ...
    erp_cursor = _db.erp_items.find(query)
    if sort_by_name:
        erp_cursor = erp_cursor.sort("item_name", 1)
    erp_items = await erp_cursor.skip(skip).limit(limit).to_list(limit)
    items = list(erp_items if isinstance(erp_items, list) else [])

    remaining = max(0, limit - len(items))
    manual_items = _manual_items_collection()
    if remaining > 0 and manual_items is not None:
        manual_skip = max(0, skip - await _db.erp_items.count_documents(query))
        manual_cursor = manual_items.find(query)
        if sort_by_name:
            manual_cursor = manual_cursor.sort("item_name", 1)
        manual_results = await manual_cursor.skip(manual_skip).limit(remaining).to_list(remaining)
...
async def get_item_batches(
    ...
    if not batches:
        ...
        manual_items = _manual_items_collection()
        if manual_items is not None:
            manual_batches = await manual_items.find(query).to_list(length=100)
            if isinstance(manual_batches, list):
                mongo_batches.extend(manual_batches)
...
async def get_all_items(
    ...
    if search and search.strip():
        ...
        total = await _count_items_across_catalogs(query)
        skip = (page - 1) * page_size
        items = await _find_items_across_catalogs(query, skip=skip, limit=page_size)
    ...
    total = await _count_items_across_catalogs({})
    skip = (page - 1) * page_size
    items = await _find_items_across_catalogs(
        {},
        skip=skip,
        limit=page_size,
        sort_by_name=True,
