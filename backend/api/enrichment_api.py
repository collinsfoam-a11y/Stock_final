...
async def check_data_completeness(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to check completeness: {e!s}"
        ) from e
...
async def get_enrichment_statistics(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get enrichment stats: {e!s}"
        ) from e
...
async def get_incomplete_items(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get incomplete items: {e!s}"
        ) from e
