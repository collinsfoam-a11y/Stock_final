...
async def get_count_lines(
    ...
    *,
    ...
    if scanned >= MAX_VERIFIED_SCAN:
        logger.warning(
            "Verified-filter query hit MAX_VERIFIED_SCAN ceiling for session %s. "
            "Some matching lines may be omitted from pagination.",
            session_id,
        )
...
async def check_item_counted(
    ...
    try:
        ...
        if current_user.get("role") == "staff":
            staff_participants = (
                set(session.get("participants") or [])
                | set(session.get("assigned_users") or [])
                | set(session.get("counters") or [])
            )
            staff_single = {
                session.get("staff_user"),
                session.get("assigned_to"),
                session.get("assigned_staff"),
            } - {None}
            allowed_staff = staff_participants | staff_single
            username = current_user.get("username")
            if (
                allowed_staff
                and username not in allowed_staff
                and username != session.get("created_by")
            ):
