...
from backend.services.item_master_lookup import find_item_master_by_barcode_or_code
...
MAX_COUNTED_ITEM_LINES_RETURNED = 200
...
async def _resolve_item_name_for_draft(db: Any, line_data: CountLineCreate) -> Optional[str]:
    ...
    erp_item = await find_item_master_by_barcode_or_code(
        db,
...
async def _find_erp_item_for_count_line(
    ...
    return await find_item_master_by_barcode_or_code(
        db,
        barcode=line_data.barcode,
        item_code=line_data.item_code,
    )
...
async def check_item_counted(
    ...
    try:
        ...
        item_totals = await read_router.get_session_item_totals(
            session_id=session_id,
            item_code=item_code,
            endpoint="count-lines/check",
        )

        # Return a bounded sample of matching lines; aggregate totals come from the read router.
        cursor = db.count_lines.find({"session_id": session_id, "item_code": item_code}).limit(
            MAX_COUNTED_ITEM_LINES_RETURNED
        )
        count_lines = await cursor.to_list(length=MAX_COUNTED_ITEM_LINES_RETURNED)
        ...
        return {
            ...
            "count_lines_truncated": len(count_lines) >= MAX_COUNTED_ITEM_LINES_RETURNED,
            "aggregate_source": item_totals["source"],
            "item_totals": {
                "total_qty": item_totals["total_qty"],
                "source": item_totals["source"],
            },
            "batch_totals": item_totals["batch_totals"],
        }
...
async def check_serial_uniqueness(
    ...
    """
    Check whether a serial number has already been counted for the same item.
    ...
    for candidate in candidates:
        ...
        legacy_serial = await db.item_serials.find_one(
            {
                "$and": [
                    {
                        "$or": [
        ...
        if legacy_serial:
            return {"exists": True, "scope": "item", **legacy_serial}
        ...
        if count_line:
            return {"exists": True, "scope": "item", **count_line}
...
async def delete_count_line(
    ...
    """Archive a count line (requires supervisor override)."""
    if current_user["role"] not in ["supervisor", "admin"]:
        raise HTTPException(status_code=403, detail="Insufficient permissions")
    ...
    try:
        ...
        write_service = _get_count_line_write_service(db)
        archived_at = _current_timestamp()
        result = await write_service.process_write(
            {
                "operation": "update_one",
                "filter": {"_id": count_line["_id"]},
                "update": {
                    "$set": {
                        ...
                        "archive_reason": "supervisor_removed_count_line",
                        "status": "superseded",
                        "superseded_at": archived_at,
                        "superseded_by": current_user.get("username"),
        ...
        return {"success": True, "message": "Count line archived successfully"}
...
async def check_item_scan_status(
    ...
    db = _get_db_client()
    try:
        return await InventoryReadRouter(db).get_session_item_scan_status(
            session_id=session_id,
            item_code=item_code,
