...
async def get_damage_case(
    ...
    if not case:
        raise HTTPException(
            ...
            detail={"success": False, "error": {"code": "NOT_FOUND", "message": "Damage case not found"}},
        )
    case.pop("_id", None)
...
async def decide_damage_case(
    ...
    update: dict[str, Any] = {
        "return_status": payload.return_status.value if payload.return_status else case.get("return_status"),
