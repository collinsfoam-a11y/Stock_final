...
class CreateSKUFromUnknownRequest(BaseModel):
    """Create a local manual item, not an ERP master record."""
...
async def map_unknown_to_sku(
    ...
    """Map an unknown item report to an existing ERP SKU"""
    service = UnknownItemService(db)
...
async def create_sku_from_unknown(
    ...
    """Create a local manual item from an unknown item report"""
    service = UnknownItemService(db)
    ...
    return {
        "success": True,
        "message": f"Created local manual item {request.item_code} and mapped report",
        "data": result,
    }
