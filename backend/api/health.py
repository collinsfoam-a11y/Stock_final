...
def _build_mongo_pool_info() -> dict[str, Any]:
    from backend.db.mongo_options import build_mongo_client_options

    options = build_mongo_client_options()
    return {
        "max_pool_size": options.get("maxPoolSize", "unknown"),
        "min_pool_size": options.get("minPoolSize", "unknown"),
        "max_idle_time_ms": options.get("maxIdleTimeMS", "unknown"),
        "server_selection_timeout_ms": options.get("serverSelectionTimeoutMS", "unknown"),
    }
