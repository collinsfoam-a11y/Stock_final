...
async def create_session(
    ...
    existing_session = await _find_existing_session_for_employee(db, current_user["username"])
    if existing_session:
        logger.info(
...
async def finalize_session(
    ...
    return await _finalize_session_canonical(
        ...
        assessment_token=request.assessment_token if request else None,
    )
...
class FinalizationAssessmentResponse(BaseModel):
    allowed: bool
...
@router.get(
    "/{session_id}/finalize-assessment",
    response_model=FinalizationAssessmentResponse,
)
async def get_finalization_assessment(
    session_id: str,
    db: AsyncIOMotorDatabase = Depends(get_db),
    current_user: dict[str, Any] = Depends(get_current_user),
) -> dict[str, Any]:
    """
    Read-only pre-finalization assessment.

    Returns whether the session can be finalized and a structured list of
    blockers. The frontend MUST read `allowed` from this response and MUST NOT
    compute finalization eligibility locally (FI-01..FI-08 / R9.*).
    """
    if current_user.get("role") not in {"supervisor", "admin"}:
        raise HTTPException(status_code=403, detail="Supervisor access required")
    ...
    assessment = await lifecycle.assess_finalization_readiness(session_id)
...
async def pause_location_session(
    ...
    if session.get("staff_user") != username and current_user.get("role") not in {
        "supervisor",
        "admin",
    }:
