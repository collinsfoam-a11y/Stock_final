...
async def supervisor_queue(
    ...
    if queue_type_upper not in valid_queues:
        raise HTTPException(
            ...
            detail={
                "success": False,
                "error": {"code": "INVALID_QUEUE", "message": f"Valid queues: {valid_queues}"},
            },
        )
    ...
    for obs in observations:
        cards.append(
            SupervisorReviewCard(
                ...
                exception_types=[
                    e if isinstance(e, str) else str(e) for e in (obs.get("exception_types") or [])
                ],
            ).model_dump()
        )
    return {
        "success": True,
        "data": {"queue_type": queue_type_upper, "items": cards, "total": len(cards)},
    }
...
async def assign_recount(
    ...
    if not request:
        raise HTTPException(
            ...
            detail={
                "success": False,
                "error": {"code": "NOT_FOUND", "message": "Recount request not found"},
            },
        )
    return {"success": True, "data": request.model_dump()}
...
async def create_additional_location_investigation(
    ...
    if payload.response == AdditionalLocationResponse.NOT_CHECKED.value:
        await db["count_observations"].update_one(
            ...
            {
                "$set": {
                    "status": CountObservationStatus.SUPERVISOR_REVIEW.value,
                    "additional_location_response": payload.response,
                }
            },
        )
    else:
        await db["count_observations"].update_one(
            {"id": payload.observation_id},
            {
                "$set": {
                    "additional_location_response": payload.response,
                    "linked_location_task_id": doc.get("id"),
                    "status": CountObservationStatus.DISTRIBUTED_STOCK_INVESTIGATION.value,
                }
            },
        )
    ...
    pipeline = [
        {"$match": {"status": CountObservationStatus.SUPERVISOR_REVIEW.value}},
        {
            "$group": {
                ...
                "count": {"$sum": 1},
                "items": {
                    "$push": {"id": "$id", "item_code": "$item_code", "session_id": "$session_id"}
                },
            }
        },
    ]
