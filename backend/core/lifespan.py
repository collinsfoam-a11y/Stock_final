...
from backend.db.mongo_options import build_mongo_client_options, mongo_transactions_required
...
mongo_client_options: dict[str, Any] = build_mongo_client_options()
...
if not RUNNING_UNDER_PYTEST:
    db_optimizer = DatabaseOptimizer(
        mongo_client=client,
        max_pool_size=int(mongo_client_options["maxPoolSize"]),
        min_pool_size=int(mongo_client_options["minPoolSize"]),
        max_idle_time_ms=int(mongo_client_options["maxIdleTimeMS"]),
        server_selection_timeout_ms=int(mongo_client_options["serverSelectionTimeoutMS"]),
        connect_timeout_ms=int(mongo_client_options["connectTimeoutMS"]),
        socket_timeout_ms=int(mongo_client_options["socketTimeoutMS"]),
        retry_writes=bool(mongo_client_options["retryWrites"]),
        retry_reads=bool(mongo_client_options["retryReads"]),
    )
...
async def lifespan(app: FastAPI):  # noqa: C901
    ...
    try:
        ...
        if mongo_transactions_required():
            hello = await client.admin.command("hello")
            transactions_supported = bool(
                isinstance(hello, dict)
                and (hello.get("setName") or hello.get("msg") == "isdbgrid")
            )
            if not transactions_supported:
                raise RuntimeError(
                    "MongoDB transactions are required in this environment. "
                    "Use a replica set or mongos deployment, not standalone MongoDB."
                )
    except Exception as e:
