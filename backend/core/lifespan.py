...
async def _init_auth(db_instance: Any, _logger: logging.Logger) -> None:
    ...
    try:
        ...
        _logger.warning(
            "Could not initialize optional seed data "
            "(may be due to MongoDB unavailability): %s",
            exc,
        )
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        from backend.services.enterprise_audit import EnterpriseAuditService as _EnterpriseAuditService
        from backend.services.enterprise_security import EnterpriseSecurityService as _EnterpriseSecurityService
