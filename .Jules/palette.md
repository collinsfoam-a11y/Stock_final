...
## 2026-06-06 - Tab Bar Accessibility & Haptics
**Learning:** Raw `<Ionicons>` components inside bottom tab bars are not automatically hidden from screen readers, causing redundant readout alongside the tab label.
**Action:** Apply `accessibilityElementsHidden={true}`, `importantForAccessibility="no"`, and `aria-hidden={true}` directly to tab bar icons, and ensure `haptics.light()` is fired on tab selection for tactile feedback.
