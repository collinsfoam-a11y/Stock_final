...
## 2026-06-03 - Spreading getAccessibleButtonProps on direct touchables
**Learning:** The custom UI governance linter (`check-ui-governance.cjs`) checks for accessibility props on raw `TouchableOpacity` components by naively parsing the code block until the first `>` character. If the block includes a multiline `onPress` arrow function before the `getAccessibleButtonProps()` spread, the regex might truncate the match and falsely report `TOUCHABLE_MISSING_ACCESSIBILITY`. Also, `getAccessibleButtonProps` automatically provides `accessibilityRole="button"`, so adding it manually causes a TypeScript `error TS2783: 'accessibilityRole' is specified more than once`.
**Action:** When adding accessibility utilities to raw touchable components, place spread attributes like `{...getAccessibleButtonProps({ ... })}` before multiline arrow functions in the component's prop list to ensure the governance linter parses them correctly. Omit the manual `accessibilityRole="button"` prop to avoid TypeScript collisions.
